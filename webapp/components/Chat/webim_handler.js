...
var selToID
    ...
    ;
//webim 登录
function initIM(userInfo,selToID,callBack,apply=false){
  console.log('发送消息给：'+selToID);
  var Config = {
    sdkappid: 1400022701,
    accountType: 8699,
    accountMode: 0,
...
//sdk登录
function sdkLogin(userInfo, listeners, options,callback,apply=false) {
    //web sdk 登录
    webim.login(userInfo, listeners, options,
        function (identifierNick) {
            ...
            setProfilePortrait({
              ...
              //申请好友
              apply ? applyAddFriend(callback) : callback(true)
            })
        },
        function (err) {
            callback(false)
...
function convertFaceMsgToHtml(content) {
    ...
    if (faceUrl) {
        return "<img src='"+faceUrl+"' width='65px'/>";
    } else {
        return data;
    }
}
